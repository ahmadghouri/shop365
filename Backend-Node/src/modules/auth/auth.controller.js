...
async function changePassword(req, res, next) {
  try {
    const matches = await comparePassword(req.body.current_password, req.user.password);
    if (!matches) return res.status(422).json({ message: 'Current password is incorrect' });
...
module.exports = { register, login, addDetails, profile, changePassword };
