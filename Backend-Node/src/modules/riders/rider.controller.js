...
const RIDABLE = [OrderStatus.PENDING, OrderStatus.CONFIRMED, OrderStatus.PREPARING, OrderStatus.PICKED_UP, OrderStatus.OUT_FOR_DELIVERY];
const RIDER_STATUSES = [OrderStatus.PICKED_UP, OrderStatus.OUT_FOR_DELIVERY, OrderStatus.DELIVERED, OrderStatus.CANCELLED];
...
class RiderController {
  ...
  async create(req, res, next) {
    try {
      ...
      if (!businessId) {
        return res.status(400).json({ message: 'No business linked to this account' });
      }
      const { name, phone_no, password, kind, cnic, license_no, vehicle_type, vehicle_no } = req.body;

      if (!name || !phone_no) {
        return res.status(400).json({ message: 'Name and phone number are required' });
      }
      if (!password) {
        return res.status(400).json({ message: 'A login password is required so the rider can log into the mobile app' });
      }
      ...
      const existing = await Rider.findOne({ business_id: businessId, phone_no });
      if (existing) {
        return res.status(409).json({ message: 'A rider with this phone already exists for your business' });
      }
      ...
      res.status(201).json({ message: 'Rider added successfully', data: rider.toJSON() });
    } catch (error) { next(error); }
  }
  ...
  async update(req, res, next) {
    try {
      ...
      const { name, kind, cnic, license_no, vehicle_type, vehicle_no, phone_no, password } = req.body;
      ...
      if (phone_no) {
        const clash = await Rider.findOne({ business_id: rider.business_id, phone_no, _id: { $ne: rider._id } });
        if (clash) return res.status(409).json({ message: 'Another rider uses this phone' });
      ...
      if (rider.user_id) {
        ...
        if (user) {
          ...
          if (req.file || req.body.image !== undefined) user.image = rider.image;
  ...
  async remove(req, res, next) {
    try {
      ...
      if (rider.user_id) {
        await User.updateOne({ _id: rider.user_id }, { deleted_at: new Date() });
      }
  ...
  async riderOrders(req, res, next) {
    try {
      const businessId = req.user.business_id;
      if (!businessId) return res.status(403).json({ message: 'No business linked to this rider' });
      ...
      const orders = await Order.find({ _id: { $in: orderIds }, status: { $in: RIDABLE } })
      ...
      for (const item of allItems) {
        if (!itemsByOrder[item.order_id.toString()]) itemsByOrder[item.order_id.toString()] = [];
      ...
      const serialized = orders.map(o => {
        ...
        return {
          ...
          user: { _id: u._id, name: u.name, phone_no: u.phone_no, address: u.address },
  ...
  async riderUpdateStatus(req, res, next) {
    try {
      ...
      if (!RIDER_STATUSES.includes(status)) {
        return res.status(400).json({ message: `Status must be one of ${RIDER_STATUSES.join(', ')}` });
      }
      ...
      if (!match) return res.status(403).json({ message: 'Not authorized for this order' });
      ...
      const messages = {
        picked_up: { title: 'Picked Up by Rider 🛵', body: 'Your order has been picked up by the rider.' },
      ...
      if (msg) notifyUser(order.user_id.toString(), { type: 'delivery', ...msg });
