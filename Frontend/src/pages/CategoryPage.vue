<template>
  <div class="mobile-spacing lg:mt-4 lg:px-32">
    ...
    <div class="relative mb-6">
      <button @click="goBack" class="absolute left-0 top-1/2 transform -translate-y-1/2">
        <svg xmlns="http://www.w3.org/2000/svg"
          class="h-4 w-4 text-gray-800 hover:text-gray-500 transition duration-150" fill="none" viewBox="0 0 24 24"
    ...
    <div class="overflow-x-auto whitespace-nowrap mb-8">
      <button v-for="filter in filters" :key="filter" @click="filterProducts(filter)" :class="[
    ...
    <!-- Products Section -->
    <div if="filteredProducts.length > 0" class="grid grid-cols-2 lg:grid-cols-4 gap-6">
      <router-link v-for="product in filteredProducts" :key="product.id" :to="{
        ...
        <!-- Image Section -->
        <div class="w-[120px] h-[120px] overflow-hidden rounded-md flex justify-center items-center mb-4">
          <img class="w-full h-full object-contain" :src="product.image_url" alt="product image" />
        </div>
        ...
        <div class="flex flex-col justify-between flex-grow text-center w-full">
          ...
          <div :class="product.type.toLowerCase() == 'services' ? 'hidden' : 'mt-auto'
            ">
        ...
        <button v-if="product.type.toLowerCase() === 'services'" @click.prevent="openContactPopup(product)"
        ...
        <button v-else @click.prevent="addToCart(product)"
          class="group w-full mt-2 lg:mt-2 flex items-center justify-center space-x-2 px-4 py-2 bg-yellow-500 text-white font-semibold text-sm rounded-full shadow-md hover:bg-yellow-500 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-opacity-75 transition duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-lg">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div ref="loadMoreTrigger" class="h-10 w-full flex justify-center items-center mt-6">
    ...
    <div v-if="filteredProducts.length === 0 && !isLoading" class="text-center py-10">
    ...
    <div v-if="showContactPopup"
      ...
      <div class="bg-white mobile-spacing md:p-8 rounded-lg max-w-md w-full">
        ...
        <div class="flex justify-end">
          <button @click="closeContactPopup"
...
const { products, currentPage, totalPages, productsListBusinessId, number } = storeToRefs(productStore);
...
const adminPhone = computed(() => { return number.value });
