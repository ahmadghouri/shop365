<template>
  <div class="max-w-6xl mx-auto p-6 rounded-md">
    ...
    <div class="mb-6 relative">
      <div class="overflow-x-auto">
        <div class="flex gap-3 whitespace-nowrap min-w-min pb-2">
          ...
          <button @click="selectedStatus = 'pending'" :class="[
          ...
          <button @click="selectedStatus = 'preparing'" :class="[
            'px-4 py-2 rounded-full text-sm font-medium transition-colors duration-300 min-w-[120px]', // Added min-width
          ...
          <button @click="selectedStatus = 'delivered'" :class="[
            'px-4 py-2 rounded-full text-sm font-medium transition-colors duration-300 min-w-[120px]', // Added min-width
          ...
          <button @click="selectedStatus = 'cancelled'" :class="[
            'px-4 py-2 rounded-full text-sm font-medium transition-colors duration-300 min-w-[120px]', // Added min-width
    ...
    <div v-if="isModalOpen"
      ...
      <div class="w-full md:w-[800px] relative bg-[#FFFFFF] rounded-lg max-h-[90vh] overflow-hidden">
        <!-- Modal Header -->
        <div
          ...
          <div class="font-semibold text-lg md:text-xl flex items-center justify-between w-full md:w-auto">
          ...
          <div class="hidden md:inline-flex gap-4 items-center">
            <button @click="updateOrderStatus('pending')" :class="{
            ...
            <button @click="updateOrderStatus('preparing')" :class="{
              'bg-yellow-500 text-white':
            ...
            <button @click="updateOrderStatus('delivered')" :class="{
            ...
            <button @click="updateOrderStatus('cancelled')" :class="{
        ...
        <div class="px-4 md:hidden mb-4">
          <select v-model="selectedOrder.status" @change="updateOrderStatus($event.target.value)"
            class="w-full p-3 border-2 rounded-lg focus:outline-none transition-colors duration-300" :class="{
        ...
        <!-- Order Items Section -->
        <div class="mt-4 px-4 md:px-6 overflow-y-auto" style="max-height: calc(90vh - 300px)">
          <div v-if="selectedOrder.items.length > 0">
            <div v-for="item in selectedOrder.items" :key="item.id" class="relative mb-4 p-4 bg-[#E5E7EB] rounded-lg">
              <div class="flex flex-col md:flex-row gap-4">
                <!-- Product Image -->
                <div class="flex-shrink-0">
                  <img :src="item.product.image_url" alt="No image"
